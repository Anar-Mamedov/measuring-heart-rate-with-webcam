...
class GUI(QMainWindow, QThread):
    ...
    def initUI(self):
        ...
        #widgets
        self.btnStart = QPushButton("Başlat", self)
        ...
        self.btnOpen = QPushButton("Aç", self)
    ...
    def main_loop(self):
        ...
        if self.process.bpms.__len__() >50:
            if(max(self.process.bpms-np.mean(self.process.bpms))<5): #show HR if it is stable -the change is not over 5 bpm- for 3s
                self.lblHR2.setText("Kalp Hızı: " + str(float("{:.2f}".format(np.mean(self.process.bpms)))) + " bpm")
